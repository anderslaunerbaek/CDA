...
for iter=1:maxiter
    tic;      
    % Update W (add eps to the denominator to avoid dividing by zero)    
    
        
    % Update H (add eps to the denominator to avoid dividing by zero)
    ...
    cpu_time(iter)=toc;
    fprintf('iteration %2.0f objective value %8.2f  pct. explained variance %3.1f \n', iter,L(iter),(1-L(iter)/SST)*100);
end