...
while dSSE>=1e-9*SSE && iter<maxiter
        ...
        %   matrizicing.m 
        %   krprod.m
        A=matricizing(X,1)*krprod(C,B)/((B'*B).*(C'*C));
        B=matricizing(X,2)*krprod(C,A)/((A'*A).*(C'*C));
        C=matricizing(X,3)*krprod(B,A)/((A'*A).*(B'*B));

        % Sum of Square Error
        SSE=norm(matricizing(X,1)-A*krprod(C,B)','fro')^2;
        dSSE=SSE_old-SSE;
